...
def send_media(filename, sender, phone_id):
    ...
    if response1.status_code == 200:
        ...
        print("send_media function failed to send image")
        ...
        for i in product_images:
            if f'{i}_image' in reply:
                ...
                try:
                    ...
                    send("An error occurred while loading the image", sender, phone_id)
                send_media(product_path, sender, phone_id)
                send(reply, sender, phone_id)
                break
    else:
        send(reply, sender, phone_id)
...
def process_message_data():
    global data_store, phone_id_store
    if data_store and phone_id_store:
        message_handler(data_store.pop(0), phone_id_store.pop(0))

scheduler.add_job(process_message_data, 'interval', seconds=5)

@app.route("/webhook", methods=["POST"])
def webhook():
    data, phone_id = take_data(request.get_json())
    data_store.append(data)
    phone_id_store.append(phone_id)
